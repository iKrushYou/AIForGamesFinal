 ...
 */
public class NegaMaxAgent extends AIAgent {
    public NegaMaxAgent(String name, int maxDepth, int maxTime) {
        this.name = name;
    ...
    public int getMove(BoardState boardState, int player) {
        return getNegaMaxMove(boardState, player);
    }

    private int getNegaMaxMove(BoardState state, int player) {
        ...
        for (int i = 0; i < possibleMoves.size(); i++) {
            ...
            int moveValue = -maxValue(possibleMoves.get(i), -1000000, 1000000, 1, GameBoard.otherPlayer(player));
    ...
    private int maxValue(BoardState state, int a, int b, int currentDepth, int player) {
        ...
        for (int i = 0; i < possibleMoves.size(); i++) {
            v = Math.max(v, -maxValue(possibleMoves.get(i), a, b, currentDepth++, GameBoard.otherPlayer(player)));
            if (v >= b) {
                return v;
            }
            a = Math.max(a, v);
